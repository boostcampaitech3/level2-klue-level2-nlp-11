...
class Mlm_Dataset_for_R(torch.utils.data.Dataset):
    def __init__(self, tokenized_dataset, labels, train=True):
        self.tokenized_dataset = tokenized_dataset
        self.labels = labels
        self.train = train
...
def random_masking_df(input_data, tokenizer, threshold=0.8, random_state=42, shuffle=True):
    ...
    masked_new_sentence_lst = []
    for origin_sentence in origin_data['sentence']:        
        origin_sentence = origin_sentence.replace('[CLS]','').replace('[SEP]','').strip()
        sub_obj = []
        flag = 0
        flagg = 0
        ...
        if will_masking_prop <= threshold:
            encoded_sentence_ids = tokenizer(origin_sentence)['input_ids']
            for i in range(len(encoded_sentence_ids)):
                if encoded_sentence_ids[i] == 31500 or flag == 1:
                    sub_obj.append(i)
                    flag = 1
                    if encoded_sentence_ids[i] == 31501:
                        flag = 0
                elif encoded_sentence_ids[i] == 31502 or flagg == 1:
                    sub_obj.append(i)
                    flagg = 1
                    if encoded_sentence_ids[i] == 31503:
                        flagg = 0
            sentence_seq_len = len(encoded_sentence_ids)
            n_masks_premise = min(3, max(1, int(sentence_seq_len*mask_prop_in_1sentence)))# 최소 1개 ~ 최대 3개
            mask_indices = set()

            while len(mask_indices)<n_masks_premise:
                a = random.randrange(1, sentence_seq_len-1)
                if a in sub_obj:
                    while a in sub_obj:
                        a = random.randrange(1, sentence_seq_len-1)
                mask_indices.add(a)
            ...
            for ind in list(mask_indices):
                encoded_sentence_ids[ind] = 4 # replace to mask 
            new_sentence = tokenizer.decode(encoded_sentence_ids)#.replace('[CLS]','').replace('[SEP]','').strip()
            masked_new_sentence_lst.append(new_sentence)
        else:
            origin_sentence = origin_sentence#.replace('[CLS]','').replace('[SEP]','').strip()

            masked_new_sentence_lst.append(origin_sentence)
    origin_data['sentence'] = masked_new_sentence_lst
